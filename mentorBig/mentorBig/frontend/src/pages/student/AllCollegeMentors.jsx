...
const AllCollegeMentors = () => {
  ...
  // Function to get year from semester
  ...
  useEffect(() => {
    const results = mentors.filter(
      (mentor) =>
        ...
        mentor.collegeName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const sendRequest = async (mentorId) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/collegeMentor/addRequests",
        {
      ...
      if (response.status === 200) {
        ...
        const updatedMentors = mentors.map((mentor) => {
          if (mentor._id === mentorId) {
            return {
              ...mentor,
              requests: [...mentor.requests, { _id: student._id }],
            };
          }
          return mentor;
        });
  ...
  const getRequestButton = (mentor) => {
    ...
    switch (status) {
      case "pending":
        return (
          <div className="w-full mt-4 bg-yellow-600 text-white py-2 rounded-lg flex items-center justify-center">
      ...
      case "accepted":
        return (
          <div className="w-full mt-4 bg-green-600 text-white py-2 rounded-lg flex items-center justify-center">
      ...
      default:
        return (
          <button
            ...
            className="w-full mt-4 bg-purple-600 hover:bg-purple-700 text-white py-2 rounded-lg transition-colors flex items-center justify-center"
          >
            {requestLoading ? (
              <>
                <svg
  ...
  const matchWithAI = async () => {
    ...
    try {
      ...
      const studentData = {
        ...
        year: getYearFromSemester(user.semester), // Convert semester to year
        course: user.course,
      ...
      // Find the actual mentor objects from our database that match the AI results
      const actualMatchedMentors = mentors.filter((mentor) =>
        matchedMentorsData.some(
          (matchedMentor) => matchedMentor.email === mentor.email
        )
      );

      setMatchedMentors(actualMatchedMentors);
      setFilteredMentors(actualMatchedMentors);
    } catch (error) {
      console.error("Error with AI matching:", error);
      alert("AI matching failed. Using fallback matching...");

      // Fallback: Randomly select 2 mentors
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-900 text-gray-100">
        <div className="container mx-auto px-4 py-8">
          {requestSuccess && (
            <div className="fixed top-20 right-4 z-50 bg-green-600 text-white px-4 py-2 rounded-md shadow-lg">
              {requestSuccess}
            </div>
          )}

          <div className="flex flex-col md:flex-row justify-between items-center mb-8 mt-[50px] gap-4">
            <h1 className="text-2xl font-bold text-white">College Mentors</h1>
          ...
          {matchedMentors.length > 0 && (
            <div className="mb-6 p-4 bg-gradient-to-r from-blue-900 to-purple-900 rounded-lg border border-blue-500">
              <h3 className="text-lg font-semibold text-white flex items-center gap-2">
                ...
                AI Matched College Mentors for You
              </h3>
              <p className="text-blue-200 text-sm mt-1">
                Based on your academic profile and preferences, we found these
                perfect matches!
              </p>
            </div>
          )}
                  ...
                  <div className="p-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="flex items-center text-gray-300">
                        ...
                        <span className="ml-1 text-white">{mentor.email}</span>
                      </p>
                    </div>
          ...
          <div className="bg-gray-800 rounded-xl max-w-2xl w-full p-6 relative">
            ...
            <div className="flex items-center mb-6">
              ...
              <div>
                ...
                <p className="text-purple-400">College Mentor</p>
